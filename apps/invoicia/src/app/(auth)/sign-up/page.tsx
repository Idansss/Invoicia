...
export default function SignUpPage() {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      toast.info("Google sign-up is not available yet. Please use email and password.");
    } catch {
    ...
    <div className="flex min-h-screen">
      <div className="hidden lg:flex lg:w-1/2 flex-col justify-between bg-foreground text-background p-12">
        ...
        <div className="space-y-8">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-balance">Start sending invoices in minutes.</h1>
            <p className="mt-3 text-base opacity-70 leading-relaxed">
              Join thousands of businesses who trust Invoicia for professional invoicing, automated reminders, and global compliance.
            </p>
          </div>
          <div className="space-y-4">
            {[
              { icon: Zap, text: "Free 14-day trial, no credit card required" },
              { icon: Globe, text: "Multi-currency & Peppol compliance" },
              { icon: Shield, text: "Enterprise-grade security" },
              { icon: CheckCircle2, text: "Unlimited invoices on all plans" },
            ].map((item) => (
        ...
        <Card className="w-full max-w-md border-0 shadow-none lg:border lg:shadow-sm">
          ...
          <CardContent className="space-y-4">
            <div className="relative w-full">
              <Button
                variant="outline"
                className="w-full h-11 font-medium bg-transparent opacity-50 cursor-not-allowed"
              ...
              <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 rounded-full border border-border bg-muted px-2 py-0.5 text-[10px] font-medium text-muted-foreground">
                Coming soon
              </span>
            </div>
